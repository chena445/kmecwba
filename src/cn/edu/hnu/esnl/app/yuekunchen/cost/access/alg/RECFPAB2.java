...
///
		double appEnergyMin = 0;
		LinkedHashMap<Task, Double> taskEnergyMinMap = new LinkedHashMap<>();

		for (Task currentTask : g0.getScheduledSequence()) {
		    Double taskEnergyMin = Double.MAX_VALUE;
		    
		    for (int i = 1; i < givenProcessorList0.size(); i++) {
		        Processor p = givenProcessorList0.get(i);
		        // 使用能量最低频率（F_ee）计算最小能量
		        double energy = EnergyService.getEnergy(currentTask, p, p.getF_ee());
		        if (energy < taskEnergyMin) {
		            taskEnergyMin = energy;
		        }
		    }
		    taskEnergyMinMap.put(currentTask, taskEnergyMin);
		    appEnergyMin = appEnergyMin + taskEnergyMin;
		}
	////	
		...
		for (int y = 0; y < g0.getScheduledSequence().size(); y++) {
			...
			for (int z = y + 1; z < g0.getScheduledSequence().size(); z++) {
				...
				// ===== 使用RECFPAB公式 =====
				double AB_G = appCostBudget - appCostMin;
		        Task unTask = g0.getScheduledSequence().get(z);	       	       
		        double alpha = 0.8;  // 能量系数（可调参数）
		        double beta = 1 - alpha;  // 成本系数	        
		        double taskEnergyMin = taskEnergyMinMap.get(unTask);
		        double taskCostMin = unTask.getRMin();	        
		        // RECFPAB预分配公式
		        double costAvailT = taskCostMin + AB_G * (
		            alpha * (taskEnergyMin / appEnergyMin)+beta * (taskCostMin / appCostMin)
		        );
		...
		System.out.println("RECFPAB2's  cost:" + DoubleUtil.format4(totalCost));
		System.out.println("RECFPAB2's  energy:" + DoubleUtil.format4(totalEnergy));
		System.out.println("RECFPAB2's  schedule length:" + DoubleUtil.format(sl));
		System.out.println("");
