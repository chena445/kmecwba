 ...
 * 
 * 
 *///来自2023年的论文 Reliability-Aware and Energy-Efficient Workflow Scheduling in IaaS Clouds
public class MECWS_REWS  {
	...
	public static Application exe(List<Processor> givenProcessorList, Integer[][] compMartrix, Integer[][] commMartrix, double appCostBudget, Boolean... print) throws IOException,
		...
		for (int y = 0; y < g0.getScheduledSequence().size(); y++) {
			...
			for (int x = 0; x < y; x++) {
				...
				appAssignedCost = appAssignedCost + g0.getScheduledSequence().get(x).getAR();//已经调度的任务用掉的花费
			...
			for (int z = y + 1; z < g0.getScheduledSequence().size(); z++) {
				...
				int n=g0.getScheduledSequence().size();
				//double costAvailT = unTask.getRMin()/appCostMin*costAvailG+unTask.getRMin();
				
			//	double costAvailT = unTask.getRMin()*appCostBudget/appCostMin;//任务的可用花费
				//double costAvailT = unTask.getRMax()* (appCostBudget/appCostMax);
				//double costAvailT = unTask.getRMax()* ( Math.pow((appCostBudget/appCostMax),1d/(n)));		
				double costAvailT = unTask.getRMax()-((appCostMax-appCostBudget)/n);
				//double costAvailT = unTask.getRMax()*( Math.pow((appCostBudget/appCostMax),1d/(n)));		
			
				
				appUnassignedCost = appUnassignedCost + costAvailT;//所以未调度的任务的预置花费

			}

				
			
			double taskCostBudget = appCostBudget - appAssignedCost - appUnassignedCost;//当前任务可用的调度花费预算
			...
			//System.out.println("虚拟机的个数" + givenProcessorList0.size());
			...
			for (int ii = 1; ii < givenProcessorList0.size(); ii++) {
				...
				//System.out.println("MECABP任务: "+currentTask.getName() + " 选择虚拟机" + actualProcessor + " taskactualcost："+actualCost + " 实际频率" + actualF + " 能量消耗：" + actualEnergy + " 任务预算" + taskCostBudget);	
			...
			if (print!=null&&print[0]==true) {
			 ...
			 totalB= totalB+taskCostBudget;
		 //   System.out.println("任务预算"+taskCostBudget+"任务成本"+currentTask.getAR());
			//System.out.println("任务预算"+ totalB+"任务成本"+currentTask.getAR());
		...
		System.out.println("MECWS_REWS 's  cost:" + DoubleUtil.format4(totalCost));
		System.out.println("MECWS_REWS 's  energy:" + DoubleUtil.format4(totalEnergy));
		System.out.println("MECWS_REWS 's  schedule length:" + DoubleUtil.format(sl));
		System.out.println("");
