 ...
 * 
 * 
 *///来自2019年的BDAS算法all in论文 Budget and Deadline Aware e-Science Workflow Scheduling in Clouds
public class MECWS_BDAS   {
	...
	public static Application exe(List<Processor> givenProcessorList, Integer[][] compMartrix, Integer[][] commMartrix, double appCostBudget, Boolean... print) throws IOException,
		...
		for (int y = 0; y < g0.getScheduledSequence().size(); y++) {
			...
			//	double costAvailT = unTask.getRMin()*appCostBudget/appCostMin;//任务的可用花费
				double costAvailT = unTask.getRMax()* ( Math.pow((appCostBudget/appCostMax),1d/(1)));		
						//appCostBudget/////-(appCostMax-appCostBudget)/ g0.getScheduledSequence().size();
		...
		System.out.println("MECWS_BDAS 's  cost:" + DoubleUtil.format4(totalCost));
		System.out.println("MECWS_BDAS 's  energy:" + DoubleUtil.format4(totalEnergy));
		System.out.println("MECWS_BDAS 's  schedule length:" + DoubleUtil.format(sl));
		System.out.println("");
