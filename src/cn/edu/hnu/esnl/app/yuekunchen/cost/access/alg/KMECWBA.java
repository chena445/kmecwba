...
public class KMECWBA {
	...
	public static Application exe(List<Processor> givenProcessorList, Integer[][] compMartrix, Integer[][] commMartrix, double appCostBudget, Boolean ...print) throws IOException, ClassNotFoundException {
		...
		int cluster_number = task_number/12 + 1; // 设置聚类数量为任务数量的/12 + 1
		KMeansRun kRun = new KMeansRun(cluster_number, taskList); // ，对数据集进行聚类
    ...
    /*任务预算分配*/ //依照rank序列对任务 找到对应的cluster 分配对应的cluster预算
		...
		for (int i = 0; i < g0.getScheduledSequence().size(); i++) {
			...
			if(currentTask.getLower_bound_ft()>sl)
	 		...
	 		System.out.println(" alg1K's  cost:" + DoubleUtil.format4(totalCost));
	 		System.out.println(" alg1K's  energy:" + DoubleUtil.format4(totalEnergy));
			System.out.println(" alg1K's  schedule length:" + DoubleUtil.format(sl));
	 		System.out.println ("");
