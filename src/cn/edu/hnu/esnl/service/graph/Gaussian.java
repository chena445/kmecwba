...
public class Gaussian {
	public static Integer[][] generateCommunicationMatrixGaussian(Integer m,int cost_lower, int cost_upper) {
		...
		for (int i = 0; i < taskNumber; i++) { // Initiate the matrix with 0.0
			for (int j = 0; j < taskNumber; j++) {
				...
				if (i == 0 && (j > 0 && j < m))
					communicationMatrix[i][j] = new Integer(cost_lower + new Random().nextInt(cost_upper));
			...
			for (int j = i; j < taskNumber; j++) {
				if (i == end + 1) {
					for (int a = i + 1; a < i + 1 + m - k; a++) {
						communicationMatrix[i][a] = new Integer(cost_lower + new Random().nextInt(cost_upper));
					}
					end = end + 1 + (m - k);
					k++;
					level--;
