	...
	public static Application exe(List<Processor> givenProcessorList, Integer[][] compMartrix, Integer[][] commMartrix, double appCostBudget, Boolean ...print) throws IOException, ClassNotFoundException {
		...
		for (int y = 0; y < g0.getScheduledSequence().size(); y++) {
			...
			for (int z = y + 1; z < g0.getScheduledSequence().size(); z++) {
				...
				double AA = g0.getScheduledSequence().get(z).getAvgW() * g0.getScheduledSequence().get(z).getRMin() /  totalTaskTime_TaskMin;
				
				double costav =appCostBudget * AA;
	
				appUnassignedCost = appUnassignedCost + costav;
			}
			...
			//System.out.println("不聚类任务分配的预算 " + taskCostBudget + "       任务成本  " +  currentTask.getRMin() + "    实际成本  " +  currentTask.getAR());
		...
		//输出调度序列
//		System.out.println("=========== MECWS_AET_II 调度序列 ===========");
...
//		System.out.println("============================================");
		...
		System.out.println(" MECWS_AET_II's  cost:" + DoubleUtil.format4(totalCost));
		System.out.println(" MECWS_AET_II's  energy:" + DoubleUtil.format4(totalEnergy));
		System.out.println(" MECWS_AET_II's  schedule length:" + DoubleUtil.format(sl));
		System.out.println ("");
