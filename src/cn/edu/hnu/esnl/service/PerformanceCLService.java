 ...
 */
public class PerformanceCLService {
	...
	public static void performance(List<Application> gs,List<Integer> criticalities, List<Processor> givenProcessorList1) {
		...
		for (Processor p : givenProcessorList1) {
			...
			for (Task task : tasks) {
				...
				totalEnergy = totalEnergy + EnergyService.getEnergy(task,p, task.getF());
		...
		for(Integer criticality: criticalities){
				...
				for (Application g : gs) {
					...
					System.out.println("g.getMakespan():"+g.getMakespan());
					System.out.println("g.getRelativeDeadline() + g.getArrivalTime():"+(g.getRelativeDeadline() + g.getArrivalTime()));
		...
		System.out.println("overall makespan:" + overMakespan);
		System.out.println("size:" + size);
		System.out.println("DMR:" + (double) missNumber / size);
