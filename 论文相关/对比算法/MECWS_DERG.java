 ...
 * 
 * 
 */   //AA2 来自2019的论文Minimizing energy consumption with reliability goal on heterogeneous embedded systems
public class MECWS_DERG{
...
System.out.println("taskTime="+ totalTaskTime);
...
//MECABP算法预算分配 未完成的任务
		...
		for (int z = y + 1; z < g0.getScheduledSequence().size(); z++) {
			...
			double costAvailG = appCostBudget-appCostMin;//总预算-最小花费
			
			Task unTask = g0.getScheduledSequence().get(z);
			
			
			//计算AA：任务平均工作时间与总工作时间的比值
			double AA = unTask.getAvgW() / totalTaskTime;
			
			//double costAvailT = unTask.getRMin()/appCostMin*costAvailG+unTask.getRMin();
			
			double costAvailT =AA*costAvailG+unTask.getRMin();//最小预算+时间比值*（总预算-最小花费）
		...
		//System.out.println("任务总预算"+b+"任务成本"+currentTask.getAR());
	}
	...
	for (int i = 0; i < g0.getScheduledSequence().size(); i++) {
		...
		System.out.println("MECWS_DERG's  cost:" + DoubleUtil.format4(totalCost));
		System.out.println("MECWS_DERG's  energy:" + DoubleUtil.format4(totalEnergy));
		System.out.println("MECWS_DERG's  schedule length:" + DoubleUtil.format(sl));
		System.out.println("");
