 ...
 * 
 */
public class TaskServiceBeanHC {
...
//+" lowerbound:"+currentTask.getLower_bound_ft());
	...
	public static void taskAllocationLowerBound(Task currentTask, List<String> opendProcessorNames, List<Processor> commomProcessorList) {
		// 在每个处理器上的值
		Double eft = Double.MAX_VALUE;
		Double ast = Double.MAX_VALUE;
		Double aft = Double.MAX_VALUE;
		Processor allocatedProcessor = null;
		//System.out.println("commomProcessorList:"+commomProcessorList);
		for (int i = 1; i < commomProcessorList.size(); i++) {

			Processor pro = commomProcessorList.get(i);
			...
			if(pro.getOpen()==false)
				continue;
			
			ESTEFT esteft = computeESTEFT(currentTask, pro, "s"); // 在处理器上的最早开始时间
	...
	public static void taskAllocationLowerBound(Task currentTask, List<String> opendProcessorNames,Processor currentProcessor, List<Processor> commomProcessorList) {
		// 在每个处理器上的值
		Double eft = Double.MAX_VALUE;
		Double ast = Double.MAX_VALUE;
		Double aft = Double.MAX_VALUE;
		Processor allocatedProcessor = null;
			...
			if(!opendProcessorNames.contains(p.getName()))
				return ;
