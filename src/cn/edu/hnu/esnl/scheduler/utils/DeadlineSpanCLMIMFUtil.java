 ...
 * 
 */
public class DeadlineSpanCLMIMFUtil {
	...
	public static void taskAllocation(String schedulingName, List<Application> apps, List<Processor> commomProcessorList, int currentTime, JdasQueue<Task> commomReadyQueue, List<Task> allocatedTasks
		...
		while (true) {
			...
			System.out.println("系统关键级："+SystemValue.system_criticality);
			...
			while (!commomReadyQueue.isEmpty()) {
				...
				if (currentTask != null) {
					...
					Processor allocatedProcessor = TaskCLServiceBean.taskAllocationDynamic(currentTask, commomProcessorList, currentTime);

					if (SystemValue.isPrint)
						System.out.println("分配的任务：" + currentTask + " 处理器" + allocatedProcessor + " 开始时间：" + currentTask.getMakespan_st() + " 完成时间" + currentTask.getMakespan_ft());
					...
					if ((currentTask.getApplication().getCriticality() > SystemValue.system_criticality)
							&& (currentTask.getMakespan_ft() - currentTask.getLower_bound_ft() - currentTask.getApplication().getArrivalTime() > currentTask.getApplication().getDeadlinespan())) {
						...
						if (SystemValue.isPrint)
							System.out.println("发现当前任务超过deadline" + currentTask + " getLower_bound_ft:" + currentTask.getLower_bound_ft() + " getMakespan_st:" + currentTask.getMakespan_st()
									+ " getMakespan_ft:" + currentTask.getMakespan_ft() + " deadline:" + (currentTask.getLower_bound_ft() + currentTask.getApplication().getDeadlinespan()));
