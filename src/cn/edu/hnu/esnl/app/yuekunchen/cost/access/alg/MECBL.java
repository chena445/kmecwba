...
public class MECBL {
	...
	public static Application exe(List<Processor> givenProcessorList, Integer[][] compMartrix, Integer[][] commMartrix, double appCostBudget, Boolean ...print) throws IOException, ClassNotFoundException {
		...
		double bl =( appCostBudget-appCostMin)/(appCostMax-appCostMin);
		...
		for (int y = 0; y < g0.getScheduledSequence().size(); y++) {
			...
			for (int z = y + 1; z < g0.getScheduledSequence().size(); z++) {
				double costBL = g0.getScheduledSequence().get(z).getRMin()+(g0.getScheduledSequence().get(z).getRMax()-g0.getScheduledSequence().get(z).getRMin())*bl;
				appUnassignedCost = appUnassignedCost + costBL;
			}
		...
		System.out.println("MECBL's  cost:" + DoubleUtil.format4(totalCost));
		System.out.println("MECBL's  energy:" + DoubleUtil.format4(totalEnergy));
		System.out.println("MECBL's  schedule length:" + DoubleUtil.format(sl));
		System.out.println("");
