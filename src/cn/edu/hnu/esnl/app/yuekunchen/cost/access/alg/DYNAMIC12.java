...
public class DYNAMIC12 {
	...
	public static Application exe(List<Processor> givenProcessorList, Integer[][] compMartrix, Integer[][] commMartrix, double appCostBudget, Boolean ...print) throws IOException, ClassNotFoundException {
		...
		for (int y = 0; y < g0.getScheduledSequence().size(); y++) {
			...
			//当前任务应用剩余预算 = 任务总预算 - 已经花出去的预算
			double currentAppCostBudget = 0d;
			currentAppCostBudget = appCostBudget - appAssignedCost;
			
			
			//当前任务应用最小预算
			double currentAppCostMin = 0d;
			double currentAppCostMax = 0d;
			double appcost=0;
			for (int z = y ; z < g0.getScheduledSequence().size(); z++) {
				currentAppCostMin = currentAppCostMin + g0.getScheduledSequence().get(z).getRMin();
				//currentAppCostMax = currentAppCostMin + currentTask.getRMax();
			   //appcost=(currentAppCostMax+currentAppCostMin)/2;			
			}
			...
			for (int z = y + 1; z < g0.getScheduledSequence().size(); z++) {
				double cost = ( g0.getScheduledSequence().get(z).getRMin() * currentAppCostBudget )  /  currentAppCostMin;
				//double costmax = ( g0.getScheduledSequence().get(z).getRMax() * currentAppCostBudget )  /  currentAppCostMax;
				
				//double cost2=(( g0.getScheduledSequence().get(z).getRMin()+ g0.getScheduledSequence().get(z).getRMax())/2 * currentAppCostBudget ) /appcost;
				appUnassignedCost = appUnassignedCost + cost;
				///appUnassignedCost = appUnassignedCost + costmax;
				//appUnassignedCost = appUnassignedCost + cost2;
			}
		...
		System.out.println("DYNAMIC's  cost:" + DoubleUtil.format4(totalCost));
		System.out.println("DYNAMIC's  energy:" + DoubleUtil.format4(totalEnergy));
		//System.out.println("DYNAMIC's  schedule length:" + DoubleUtil.format(sl));
		System.out.println ();
